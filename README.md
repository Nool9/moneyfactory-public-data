...
New raw captures are stored without transformation as deterministic `*.json.gz` files. Each 30-minute slot has a manifest containing source URLs, timestamps, hashes and byte sizes. To keep Git history small, the workflow uploads one append-only archive per slot to a daily public GitHub Release; the branch stores only `current/raw_checkpoint.json` and `gaps/gaps.jsonl`. The active 25-contract small-cap basket is frozen in `config/binance_small_caps_v2.json`; v1 remains as history. BTCUSDT and ETHUSDT depth are captured separately.
